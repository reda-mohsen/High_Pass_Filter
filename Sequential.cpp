...
//// and applies a high pass filter to the input image array to obtain an output image array
//int* highPassFilter(int* input, int width, int height, int filterSize) {
//
...
//
//	int start_s, stop_s, Sequential_TotalTime = 0; // Declare variables to measure the time taken to process the image
//
//	start_s = clock(); // Start the timer
//	// Perform any image processing tasks or calculations
//	// Apply the high pass filter
//	filteredImage = highPassFilter(imageData, ImageWidth, ImageHeight, FilterSize);
//	stop_s = clock(); // Stop the timer
//	Sequential_TotalTime += (stop_s - start_s) / double(CLOCKS_PER_SEC) * 1000; // Calculate the total time taken to process the image
//
//	createImage(filteredImage, ImageWidth, ImageHeight, 18); // Call the createImage function to save the resulting image
//	cout << "Sequential Time: " << Sequential_TotalTime << " ms" << endl; // Print the total time taken using sequential/serial
//
